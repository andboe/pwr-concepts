...
* You can't add content such as lists or admonitions.
* Any long-form content basically breaks the table layout.
* Line breaks, if they are supported at all, require HTML syntax or other workarounds.
* Alignment options are limited and again trigger changes to every line of the table.
* Formatting must be applied on a per-cell-basis.
* etc.
